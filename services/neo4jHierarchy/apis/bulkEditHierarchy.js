...
const s3 = new AWS.S3();
export const bulkEditHierarchy = async(event) => {
    try{
      devLogger("bulkEditHierarchy", event, "event");
      let userToken =null;
      ...
      const resp = await main({
        actionType: 'bulkEditHierarchyForExcel',
        nodeData: result,
        relationName: event.path.relation
      });
      return resp;
    } catch(err) {
      errorLogger("bulkEditHierarchy", err, "Error db call");
